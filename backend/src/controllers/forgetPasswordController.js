...
const authController = {
  forgotPassword: async (req, res) => {
    try {
      ...
      const resetLink = `http://localhost:4200/webhome/homepage/reset-password/${resetToken}`;
      const mailOptions = {
      ...
      res
        .status(500)
        .json({ error: "Internal server error to send the reset" });
    }
  },
  ...
  updatePassword: async (req, res) => {
    try {
      ...
      if (!hotel) {
        return res.status(404).json({ error: "Update pwd: Hotel not found" });
      }
